...
public class RecipeController {
    ...
    public ResponseEntity<?> findRecipeByIngredients(@RequestParam("ingredients") List<Long> ids, @RequestParam("order") String order) throws Exception {
        List<RecipeDto> recipeDtoList = new ArrayList<>();
        try {
            recipeDtoList.addAll(recipeService.findByIngredients(ids, order));
            return new ResponseEntity<List<RecipeDto>>(recipeDtoList, HttpStatus.OK);
        } catch (Exception e) {
            return exceptionHandling(e);
        }
    ...
    public void likeRecipe(@RequestParam("recipe_id") Long id) throws Exception {
        try{
    ...
    @GetMapping("/ingredient/findbytitleandcategory")
    public ResponseEntity<?> findIngredientByTitleAndCategory(@RequestParam("ingredient_title") String title, @RequestParam("ingredient_category") String category) throws Exception{
        List<IngredientDto> ingredientDtoList = new ArrayList<>();
        try{
            ingredientDtoList.addAll(ingredientService.findByTitleAndCategory(title, category));
            return new ResponseEntity<List<IngredientDto>>(ingredientDtoList, HttpStatus.OK);
        } catch (Exception e) {
            return exceptionHandling(e);
        }
